import { readFile, writeFile, mkdir, readdir, rm, stat, unlink, open, copyFile } from "node:fs/promises";
...
export class StateManager {
  ...
  async loadProjectConfig(): Promise<Record<string, unknown>> {
    const configPath = join(this.projectRoot, "autow.json");
  ...
  async isCompleteBookDirectory(bookDir: string): Promise<boolean> {
    ...
    const usesNewFoundationLayout =
      await this.pathExists(join(storyDir, "outline", "story_frame.md")) ||
      await this.pathExists(join(storyDir, "outline", "volume_map.md"));
    if (usesNewFoundationLayout) {
      if (!await this.pathExists(join(storyDir, "style_guide.md"))) return false;
      if (!await this.hasAnyRoleCard(storyDir)) return false;
    }
  ...
  private async hasAnyRoleCard(storyDir: string): Promise<boolean> {
    const roleDirs = [
    ...
    for (const dir of roleDirs) {
      try {
        ...
        if (entries.some((entry) => entry.endsWith(".md"))) return true;
      } catch {
        // try next role directory
      }
    }
    return false;
  }
  ...
  async rollbackToChapter(
    ...
    // 数据安全:删除前先把要丢弃章节的正文备份到 backups/,
    // 这样即便回滚后的重写失败(质量门禁/模型错误/超时),用户仍能找回原稿。
    if (discarded.length > 0) {
      const backupDir = join(
        bookDir,
        "backups",
        `pre-rollback-${new Date().toISOString().replace(/[:.]/g, "-")}`,
        "chapters",
      );
      try {
        const files = await readdir(chaptersDir);
        ...
        for (const file of files) {
          ...
          if (num > targetChapter) {
            ...
            await copyFile(join(chaptersDir, file), join(backupDir, file)).catch(() => {});
          }
        }
      } catch {
        // chapters directory missing — nothing to back up
      }
    }
