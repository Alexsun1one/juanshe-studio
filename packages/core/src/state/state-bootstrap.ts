...
export async function bootstrapStructuredStateFromMarkdown(params: {
  ...
  // 这三个 bootstrap 调用的副作用（按需创建结构化状态文件、累积 warnings/createdFiles）才是目的，
  // 返回值本身在此不再使用，因此只保留 await、丢弃未用绑定（保持行为不变）。
    ...
    return HooksStateSchema.parse({
      hooks: tableRows
        ...
        .map((row) => {
          ...
          const fallbackSixColumnShape = row.length === 6 && parseStrictIntegerCell(row[4]) === null;
          return {
            hookId,
            ...
            lastAdvancedChapter: fallbackSixColumnShape
              ? parseStrictIntegerWithWarning(row[1], warnings, `${hookId}:lastAdvancedChapter`)
              : parseStrictIntegerWithWarning(row[4], warnings, `${hookId}:lastAdvancedChapter`),
            expectedPayoff: fallbackSixColumnShape ? (row[4] ?? "") : (row[5] ?? ""),
            payoffTiming: legacyShape ? undefined : normalizeHookPayoffTiming(row[6]),
            notes: fallbackSixColumnShape ? (row[5] ?? "") : legacyShape ? (row[6] ?? "") : (row[7] ?? ""),
          };
        }),
    });
  }
