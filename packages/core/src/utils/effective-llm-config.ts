  ...
  if (!apiKey && input.requireApiKey !== false && !isApiKeyOptionalForEndpoint({ provider, baseUrl })) {
    throw new Error(
      configMode === "studio-project"
        ...
        : "JUANSHE_LLM_API_KEY not set. Add it to the project .env file. Legacy HARDWRITE_LLM_API_KEY / AUTOW_LLM_API_KEY is still accepted.",
    );
  }
...
async function readProjectConfig(root: string): Promise<Record<string, unknown>> {
  const configCandidates = ["hardwrite.json", "autow.json"];
  let configPath = "";
  let configName = "";
  for (const candidate of configCandidates) {
    ...
    try {
      ...
      // Try the next known project config name.
    }
  }
  if (!configPath) {
    throw new Error(
      `project config not found in ${root}.\nMake sure you are inside a Juanshe workspace directory.`,
    );
  }
  ...
  try {
    ...
    throw new Error(`${configName} in ${root} is not valid JSON. Check the file for syntax errors.`);
  }
}
...
async function applyCliProjectConfig(
  ...
  const envBaseUrl = stringValue(env.AUTOW_LLM_BASE_URL);
  const envService = stringValue(env.AUTOW_LLM_SERVICE) ?? (envBaseUrl ? guessServiceFromBaseUrl(envBaseUrl) : undefined);
  const envModel = stringValue(env.AUTOW_LLM_MODEL);
  ...
  await applyProjectServiceConfig(config, llm, services, input.projectRoot, diagnostics, {
    ...
    envApiKey: allowEnvEndpointOverlay ? stringValue(env.AUTOW_LLM_API_KEY) : undefined,
    envBaseUrl: allowEnvEndpointOverlay ? envBaseUrl : undefined,
    envProvider: allowEnvEndpointOverlay ? stringValue(env.AUTOW_LLM_PROVIDER) : undefined,
    cli: input.cli,
    env,
  });
}
...
async function applyLegacyEnvConfig(
  ...
  if (env.AUTOW_LLM_SERVICE) {
    llm.service = env.AUTOW_LLM_SERVICE;
    diagnostics.serviceSource = "env";
  } else if (typeof llm.service !== "string" || llm.service.length === 0) {
    llm.service = "custom";
  }

  if (env.AUTOW_LLM_PROVIDER) llm.provider = env.AUTOW_LLM_PROVIDER;
  else if (typeof llm.provider !== "string" || llm.provider.length === 0) llm.provider = "custom";
  if (env.AUTOW_LLM_BASE_URL) llm.baseUrl = env.AUTOW_LLM_BASE_URL;
  if (env.AUTOW_LLM_MODEL) {
    llm.model = env.AUTOW_LLM_MODEL;
    diagnostics.modelSource = "env";
  }
  if (env.AUTOW_LLM_API_KEY) {
    llm.apiKey = env.AUTOW_LLM_API_KEY;
...
function applyCommonEnv(
  ...
  if (env.AUTOW_LLM_TEMPERATURE) llm.temperature = Number.parseFloat(env.AUTOW_LLM_TEMPERATURE);
  if (env.AUTOW_LLM_THINKING_BUDGET) llm.thinkingBudget = Number.parseInt(env.AUTOW_LLM_THINKING_BUDGET, 10);
  if (env.AUTOW_LLM_PROXY_URL) llm.proxyUrl = env.AUTOW_LLM_PROXY_URL;
  if (env.AUTOW_LLM_API_FORMAT) llm.apiFormat = env.AUTOW_LLM_API_FORMAT;
  if (env.AUTOW_LLM_STREAM) llm.stream = parseBoolean(env.AUTOW_LLM_STREAM);
  if (env.AUTOW_DEFAULT_LANGUAGE) config.language = env.AUTOW_DEFAULT_LANGUAGE;

  const extraFromEnv: Record<string, unknown> = {};
  for (const [key, value] of Object.entries(env)) {
    if (key.startsWith("AUTOW_LLM_EXTRA_") && value) {
      extraFromEnv[key.slice("AUTOW_LLM_EXTRA_".length)] = parseEnvValue(value);
    }
  }
...
function warnIfStudioIgnoresEnv(layers: LLMEnvLayers, diagnostics: MutableDiagnostics): void {
  ...
  if (Object.keys(ignored).some((key) => key.startsWith("HARDWRITE_LLM_") || key.startsWith("AUTOW_LLM_"))) {
    diagnostics.warnings.push("Studio 运行时不会使用 env 中的 HARDWRITE_LLM_* 配置；请在服务配置页保存 Studio 配置。旧 AUTOW_LLM_* 仍兼容。");
  }
}
