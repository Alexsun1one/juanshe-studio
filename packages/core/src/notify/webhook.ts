...
export async function sendWebhook(
  ...
  if (config.secret) {
    const signature = createHmac("sha256", config.secret)
      .update(body)
      .digest("hex");
    headers["X-HardWrite-Signature"] = `sha256=${signature}`;
    // Keep the legacy header during the fork transition so existing webhooks do not break.
    headers["X-AutoW-Signature"] = `sha256=${signature}`;
  }
