...
export interface AgentSessionConfig {
  ...
  apiKey?: string;
  /** Allow the read tool to read absolute paths outside projectRoot/books. Defaults to false; set AUTOW_AGENT_ALLOW_SYSTEM_READ=1 to enable. */
  allowSystemFileRead?: boolean;
  /** 改写护栏:每次直接动作品文件的工具调用前执行;抛错即拦截(如写作流水线在跑时)。 */
...
/** 改写护栏:写作流水线在跑时,拦住会直接动作品文件的工具(由调用方注入,如 studio 检查 active run)。
 *  抛错即拦截 —— 包装器把抛错转成给"猫"看的友好提示,而不是炸成工具异常。 */
export type MutationGuard = (op: string) => void | Promise<void>;

function guardMutationTool<T extends AgentTool<any>>(tool: T, guard: MutationGuard | undefined): T {
  if (!guard) return tool;
  const guarded = {
    ...tool,
    execute: async (...args: Parameters<T["execute"]>) => {
      try {
        await guard(tool.name);
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        return { content: [{ type: "text", text: msg }], details: undefined };
      }
      return (tool.execute as (...a: unknown[]) => unknown)(...args);
    },
  };
...
function createAgentToolsForMode(params: {
  ...
  const guard = params.mutationGuard;
  return [
    subAgentTool,
    createReadTool(params.projectRoot, { allowSystemPaths: params.allowSystemFileRead }),
    guardMutationTool(createWriteTruthFileTool(params.pipeline, params.projectRoot, params.bookId), guard),
    guardMutationTool(createRenameEntityTool(params.pipeline, params.projectRoot, params.bookId), guard),
    guardMutationTool(createPatchChapterTextTool(params.pipeline, params.projectRoot, params.bookId), guard),
    guardMutationTool(createEditTool(params.projectRoot), guard),
    guardMutationTool(createWriteFileTool(params.projectRoot), guard),
...
async function runAgentSessionUnlocked(
  ...
  const allowSystemFileRead = config.allowSystemFileRead ?? envFlagEnabled(process.env.AUTOW_AGENT_ALLOW_SYSTEM_READ, false);
  ...
  if (!cached) {
    ...
    const agent = new Agent({
      initialState: {
        ...
        tools: createAgentToolsForMode({ pipeline, bookId, projectRoot, allowSystemFileRead, mutationGuard: config.mutationGuard }),
