...
- **状态方向必须与正文一致（铁律）**：伏笔/关系的状态变化方向必须跟正文走。如果正文让某条线**加深、强化、升级、或被进一步锚定**，它的状态只能保持或往“更紧 / 已锚定 / 推进”方向走，**绝不能下调或弱化**（例如把 anchored 降回 open 这种降级方向就是错的，会与正文直接矛盾）。只有当正文**明确**显示这条线**松动、被推翻、被搁置或不再成立**时，才允许下调状态。方向写反 = 与正文矛盾 = 被一致性校验判 FAIL。
- **正文写到的身体/状态变化必须落进状态卡**：主角或在场角色在正文里出现的受伤、体力/血糖/中毒、意识波动、关键物品的共振/异动等具体变化，都要写进 currentStatePatch.protagonistState 或对应状态字段，不能只写在正文里却漏记。
- **持久状态必须累积、不能被新状态覆盖丢失（铁律）**：主角身上**尚未在正文里解除**的伤、债、人情、把柄/暴露风险、未还的承诺，即使本章没再提，也**必须继续保留在 currentStatePatch.protagonistState / currentConstraint 里**（与本章新状态并列写，例如"断两根肋骨未愈 + 本章精神紧绷"）。currentStatePatch 是整字段覆盖更新——你这一章漏写的持久状态就等于把它从书里抹掉了。只有当正文**明确**写出某条状态被解除（伤好了、债还清、把柄销毁）时，才能把它从状态卡拿掉。绝不允许因为"本章没提"就让上一章的伤/债凭空消失——那会与后续正文直接矛盾、被一致性校验判 FAIL。
...
## 结算自检清单（对照一致性审稿 6 类，逐条核对，并把结论写进 POST_SETTLEMENT）

产出 RUNTIME_STATE_DELTA 之前，先拿正文逐条过下面 6 项——这正是后续一致性审稿会逐条检查的维度，在源头堵住，章节才不会被判 critical 打回：

1. **有据可依**：你写进状态卡 / 钩子 / 账本的每一处变化，正文里是否都有明确文字支撑？没有支撑的一律不写。
2. **不漏变化（最常踩的坑）**：正文里实际发生的每一项变化都要落进对应文件——逐个点名核对:主角及在场角色的身体/状态(受伤、体力、血糖、中毒、意识波动)、关键物品的获得/消耗/易手/共振异动、位置移动、关系变化、谁知道了什么(信息边界)。**漏一项就会被审稿判 critical。**
3. **时序合理**：位置转移有过渡吗？受伤/恢复有对应的时间流逝吗？不得出现“瞬移”“秒愈”。
4. **钩子账实**：本章触及的每个钩子，状态变化方向必须与正文一致（正文让它加深 → 标 anchored/推进，**绝不反向降级**）；消失的钩子必须标回收；新钩子必须有正文依据。
5. **不改前章**：改动只能反映“本章”发生的事，不得把本属于前几章的事追溯写进来。
6. **键设不冲突**：编号规则 / 命名 / 身份 / 地点 / 关系标签，与正文和既定设定不得冲突。

6 条全部核对、修正后再输出 RUNTIME_STATE_DELTA。**POST_SETTLEMENT 必须逐条给出这 6 项的自检结论（每条一句：核对结果 / 发现并修正了什么）。**
...
（先逐条作答"结算自检清单"6 项：每条一句写核对结果 / 修正了什么；再简要说明本章状态变动、伏笔推进、结算注意事项。允许 Markdown 表格或要点）
