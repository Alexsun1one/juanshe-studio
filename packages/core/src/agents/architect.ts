...
export class ArchitectAgent extends BaseAgent {
  ...
  async generateFoundation(
    ...
    const sysContent = langPrefix + systemPrompt + revisePrompt;
...
${trimmed}\n`;
  ...
  /**
   * 把 === SECTION: X === 的标题解析成规范 key。
   * 关键修复:normalizeSectionName 会把中文名洗成空串 —— 中文模型写中文小说时常把
   * 段标题写成中文(=== SECTION: 故事框架 ===),于是规范名变空 → 五段全判"缺失" → 建书崩。
   * 这里先走英文规范名 + 别名表,再按中文/模糊关键词兜底映射到规范 key。
   */
  private resolveSectionKey(rawName: string): string {
    const norm = this.normalizeSectionName(rawName);
    const ALIAS: Record<string, string> = {
      storyframe: "story_frame", story_frame: "story_frame",
      storybible: "story_bible", story_bible: "story_bible",
      volumemap: "volume_map", volume_map: "volume_map",
      volumeoutline: "volume_outline", volume_outline: "volume_outline",
      role: "roles", roles: "roles",
      bookrules: "book_rules", book_rules: "book_rules",
      pendinghooks: "pending_hooks", pending_hooks: "pending_hooks",
      hooks: "pending_hooks", hook: "pending_hooks",
      rhythmprinciples: "rhythm_principles", rhythm_principles: "rhythm_principles",
      currentstate: "current_state", current_state: "current_state",
    };
    if (norm && ALIAS[norm]) return ALIAS[norm];
    // 中文 / 模糊兜底(段标题是单一概念,按主关键词匹配)
    const raw = rawName.toLowerCase();
    if (/故事框架|故事骨架|世界框架|story.?frame/.test(raw)) return "story_frame";
    if (/故事圣经|设定集|story.?bible/.test(raw)) return "story_bible";
    if (/卷地图|卷纲|卷线|分卷|卷规划|volume.?map/.test(raw)) return "volume_map";
    if (/卷大纲|卷outline|volume.?outline/.test(raw)) return "volume_outline";
    if (/角色|人物/.test(raw)) return "roles";
    if (/设定规则|世界规则|创作规则|硬规则|book.?rules?/.test(raw)) return "book_rules";
    if (/伏笔|钩子|悬念|pending.?hooks?/.test(raw)) return "pending_hooks";
    if (/节奏原则|节奏/.test(raw)) return "rhythm_principles";
    if (/当前状态|初始状态|现状|current.?state/.test(raw)) return "current_state";
    return norm;
  }
