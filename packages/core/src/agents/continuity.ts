...
const DIMENSION_LABELS: Record<number, { readonly zh: string; readonly en: string }> = {
  ...
  38: { zh: "主设定保真", en: "Premise Fidelity Check" },
};
...
description 中要明确引用 hook_id，并把状态列中 stale / blocked 的原文标记字面抄进去。本维度只审结构，不评价伏笔文笔。`;
    ...
    case 38:
      return language === "en"
        ? "Cross-check author_intent, story_frame, current_focus, role cards, and the Chapter Control Inputs. The chapter must keep the book's defining promise visible through at least one on-page signal: protagonist identity, core edge and limitation, long-range mystery movement, relationship promise, payoff with cost, or worldview rule in action. If the local incident could belong to a generic different book and the defining premise disappears, mark critical."
        : "对照 author_intent、story_frame、current_focus、角色卡与本章控制输入。成稿必须至少让一个全书核心承诺在页面上可见：主角身份反差、核心能力及限制、长线谜团推进、关系承诺、带代价的爽点收益、或世界规则落地。若本章局部事件脱离这些信号，读起来像另一本普通书，标为 critical。";
    case 34:
    case 35:
    case 36:
    case 37: {
...
function buildDimensionList(
  ...
  activeIds.add(38); // 主设定保真 — universal
...
Premise fidelity is structural, not stylistic. When the control inputs include author intent, story frame, role cards, or a core promise, the chapter must visibly carry at least one defining premise signal on page. If the chapter follows the latest incident but hides the protagonist concept / core edge / long-range promise so thoroughly that it reads like a different generic book, return a critical issue under Premise Fidelity Check.
...
主设定保真属于结构审稿，不属于文笔审稿。只要本章控制输入里包含 author_intent、story_frame、角色卡或全书核心承诺，成稿就必须至少显化一个主设定信号。若章节只是顺着最新局部事件推进，却把主角概念 / 核心能力 / 长线承诺藏到读者看不见，读起来像另一本普通书，必须在“主设定保真”下给 critical。
...
综合评分，不要因为单一小问题大幅拉低分数。`;
    ...
    //  - block 3:任务指令,短且不变 — 不缓存
    // OpenAI 路径会 flatten,行为不变;只在 Anthropic 系真省 token
...
${hooksBlock}${volumeSummariesBlock}${subplotBlock}${emotionalBlock}${matrixBlock}${summariesBlock}${canonBlock}${fanficCanonBlock}${reducedControlBlock}${memoBlock}${prevChapterBlock}${styleGuideBlock}\n\n`;
    ...
    const chatMessages = [
      // system 标 cache:长 38 维 audit 规则,每个 audit 调用都能命中
      { role: "system" as const, content: [{ text: systemPrompt, cache: true }] },
      // context + chapter 都标 cache;task 不缓存(短)
