...
15. 单章容量上限（硬约束，凌驾于圆心法/揭1埋2/爽点密度之上）：除非本章是明确的大高潮或群像大场面，单章默认 **场景 ≤ 3、有戏份的登场人物 ≤ 3**（纯报名字的配角不计）。圆心法、揭1埋2、爽点密集都**只在这个预算内**执行——预算用完就减视角、减新场景、减新钩子，**绝不允许为了"多视角""多埋钩""加爽点"而额外拉人进场或额外开新场景**。这一条和上面 13/14 冲突时，以本条为准。而且每份 memo 的「## 不要做」里**必须显式写出本章的场景/人物上限**（如"本章 ≤3 场景、≤3 有戏人物，不引入新配角"），给写手一条能照着核验的硬线，而不是留空
16. 反愿望清单：memo 是给写手的施工图，不是给作者打气的目标清单。"主角变强""赢得信任""推进调查""揭开真相一角"这类没有具体动作、没有具体对象、没法拍成画面的句子一律不合格——每一个目标都要落到"谁、在哪、对着什么、做了什么可观察的动作、谁挡着、最后哪个量变了"。判断标准：把这条 memo 交给写手，他能不能不靠脑补就写出场景？不能，就是还没规划完
17. 开篇多样化（硬约束）：你必须从最近摘要和上一章最后一屏里保守识别已用过的开篇类型、场景仪式、招牌意象；memo 的「## 不要做」里必须提醒写手本章换开篇类型，避开最近 5 章具象意象/细节。若同地点/同人物接续，要求开篇 100 字内给新鲜感官、信息或冲突，不许复刻上一章开场
18. 卷纲 OKR 是硬上游锚：只要输入的「本卷方向 / 卷纲硬锚」里出现「本章 must_advance KR」，本章就必须推进这个 KR；局部剧情惯性只能改“怎么发生”，不能把 KR 改没。新开支线、新 core_hook、章尾新钩子都必须服务当卷任一 KR，否则宁可不开
19. 文本气质多样化（软压力）：如果输入里有「文本多样性 / 结尾账本」，本章宏观角色必须同时包含文本 register/tempo 目标。连续阴郁内省、慢观察、同型结尾、同一小动作或同一客人画像模板时，memo 的「## 当前任务」「## 日常/过渡承担什么任务」或「## 不要做」必须给写手明确换档动作：温暖/明快/对话密、快一点或中速后果段、换结尾形状、换身体语言、换配角阶层/年龄/气质
...
- servesKr 写本章兑现/推进的卷级 KR id（如 KR1）；如果输入没有卷纲硬锚才写 null。只要"本卷方向"里出现"本章 must_advance KR"，就必须填写对应 id，并在正文 8 段里让这个 KR 可见
- register / tempo 是本章前向火候档位，必须照「本章 register/tempo 前向目标」填写，只能使用 register=warm|tense|bright|dialogue|gloomy|neutral 与 tempo=fast|medium|slow，不要写自由描述
- threadRefs 是 YAML 数组，内容是从输入的 pending_hooks/subplot_board 中挑出的 id
- 每个二级标题（##）必须出现，内容不能为空
- 不要在 memo 里提方法论术语（"情绪缺口"、"cyclePhase"、"蓄压"等）——直接用这本书的人物、地点、事件说事
- 不要产生正文片段或对话片段
- 如果卷纲和上章摘要冲突：摘要只决定“已经怎么发生”的连续性细节，不得废弃当卷尚未达成的 KR；若摘要显示主线没推进而卷纲要求推进，以卷纲为准，本章必须补上对应 KR。`;
...
16. Volume OKR is the hard upstream anchor: if the input "Current volume direction / hard outline anchor" contains "must_advance KR", this chapter must advance that KR. Local momentum may change how it happens, but cannot cancel the KR. Any new subplot, new core_hook, or end-hook opened this chapter must serve some KR in the current volume; otherwise do not open it.
17. Text diversity (soft pressure): if the input contains a Text Diversity / Ending Ledger brief, the macro chapter role must include its register/tempo target. When recent chapters repeat gloomy introspection, slow observation, the same ending shape, the same protagonist tic, or the same side-character portrait template, the memo must give the writer a concrete shift: warm / bright / dialogue-dense, faster or medium-consequence tempo, a different ending shape, different body language, and a different class / age / texture for new side characters.
...
- servesKr is the volume-level KR id this chapter advances/pays off (for example KR1); write null only when the input has no hard volume anchor. If "Volume direction" contains "must_advance KR", this field must name that id and the 8 memo sections must make that KR visible
- register / tempo are the forward heat controls for this chapter. Copy the provided "Chapter register/tempo forward target" exactly. Allowed values: register=warm|tense|bright|dialogue|gloomy|neutral; tempo=fast|medium|slow. Do not invent prose labels
...
- If the volume outline conflicts with the previous chapter summary: the summary controls continuity details about how events already happened, but it must not cancel any unfinished volume KR. If summaries show the mainline stalled while the volume KR requires movement, the volume KR wins and this chapter must make up that KR progress.`;
...
## Last 8 chapter summaries (compressed view)
{{recent_summaries}}

## Used opening / imagery ledger (hard constraint)
{{opening_ledger_brief}}

## Text diversity / register target (soft cadence pressure)
{{text_diversity_brief}}

{{chapter_heat_target_block}}

## What the current arc is pushing
{{current_arc_prose}}

## Dormant subplot revival hints
{{dormant_subplot_revival_hints}}

## Earlier related chapters (semantic recall — don't reuse spent beats/conflicts, don't drop long-planted threads)
{{recalled_summaries}}

## Previous audit feedback to repair in this memo
{{audit_feedback}}

## Current volume direction / hard outline anchor (must advance this KR when present)
{{volume_direction}}
...
## 开篇多样化（硬约束）
请根据上面的「上一章最后一屏」和「最近 8 章摘要」判断前几章已经用过的开篇类型、场景仪式和招牌意象；如果摘要里没有结构化列出，就只按可见文本保守判断，不要编造不存在的数据。本章 memo 必须明确提醒写手：换一种开篇类型，避开最近 5 章已出现的具象意象/细节；如果本章仍在同地点或同人物附近开场，开篇 100 字内必须给出新的感官、信息或冲突，打破重复感。

{{opening_ledger_brief}}

## 文本气质多样化（软 cadence 压力）
{{text_diversity_brief}}
...
## 历史相关章（语义召回 —— 别重复用过的爽点/冲突，别忘了早年埋的线）
{{recalled_summaries}}

## 上轮审计反馈（本章 memo 必须转成修复动作）
{{audit_feedback}}

## 本卷方向 / 卷纲硬锚（出现 must_advance KR 时，本章必须推进）
{{volume_direction}}
...
export interface PlannerUserMessageInput {
  ...
  readonly progressDashboard?: string;
  readonly openingLedgerBrief?: string;
  readonly textDiversityBrief?: string;
  readonly chapterHeatTarget?: string;
  readonly currentArcProse: string;
  readonly dormantSubplotRevivalHints?: string;
...
export function buildPlannerUserMessage(input: PlannerUserMessageInput): string {
  ...
  const filled = template
    ...
    .replaceAll("{{progress_dashboard_block}}", buildProgressDashboardBlock(input.progressDashboard ?? "", language))
    ...
    .replaceAll("{{opening_ledger_brief}}", input.openingLedgerBrief?.trim() || (language === "en" ? "(none yet)" : "（暂无结构化开篇账本）"))
    .replaceAll("{{text_diversity_brief}}", input.textDiversityBrief?.trim() || (language === "en" ? "(none yet)" : "（暂无文本多样性账本）"))
    .replaceAll("{{chapter_heat_target_block}}", input.chapterHeatTarget?.trim() || "")
    .replaceAll("{{current_arc_prose}}", input.currentArcProse)
    .replaceAll("{{dormant_subplot_revival_hints}}", input.dormantSubplotRevivalHints?.trim() || (language === "en" ? "(none)" : "（暂无）"))
    ...
    .replaceAll("{{audit_feedback}}", input.auditFeedback?.trim() || (language === "en" ? "(no unresolved audit feedback)" : "（暂无未处理审计反馈）"))
    .replaceAll("{{recalled_summaries}}", input.recalledSummaries ?? "")
    .replaceAll("{{volume_direction}}", input.volumeDirection ?? "")
...
function buildProgressDashboardBlock(dashboard: string, language: "zh" | "en"): string {
  const trimmed = dashboard.trim();
  if (!trimmed) return "";
  if (trimmed.startsWith("## ")) {
    return `${trimmed}\n`;
  }
  if (language === "en") {
    return `## Whole-book progress dashboard (hard structural constraint)\n${trimmed}\n`;
  }
  return `## 全书进度仪表盘（硬结构约束）\n${trimmed}\n`;
}
