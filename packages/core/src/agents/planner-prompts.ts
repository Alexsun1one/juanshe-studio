...
## 开篇多样化（硬约束）
请根据上面的「上一章最后一屏」和「最近 3 章摘要」判断前几章已经用过的开篇类型、场景仪式和招牌意象；如果摘要里没有结构化列出，就只按可见文本保守判断，不要编造不存在的数据。本章 memo 必须明确提醒写手：换一种开篇类型，避开最近 5 章已出现的具象意象/细节；如果本章仍在同地点或同人物附近开场，开篇 100 字内必须给出新的感官、信息或冲突，打破重复感。
