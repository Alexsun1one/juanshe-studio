...
export class ReviserAgent extends BaseAgent {
  ...
  async reviseChapter(
    ...
    const heatGuard = this.buildRevisionHeatGuard(options?.chapterMemo, options?.chapterIntentData, resolvedLanguage);
    ...
    const systemPrompt = heatGuard ? `${systemPromptBase}\n\n${heatGuard}` : systemPromptBase;
    ...
    // Anthropic prompt caching:context + chapter 标 cache(audit-revise-reaudit 循环共享)
    const contextBlock = `## 当前状态卡
${currentState}
${ledgerBlock}
...
8. If the selected context includes a Text Diversity / Ending Ledger brief, preserve its register/tempo target in the revision and avoid restoring repeated ending shapes, protagonist tics, or side-character portrait templates
...
7. 坏事叠坏事，每层比上一层过分
8. 如果本章控制输入/已选上下文里有「文本多样性 / 结尾账本」，修稿后仍必须服从 register/tempo 目标，不要把结尾形状、主角小动作或配角画像模板改回近章重复款
...
8. 如果上下文里出现「文本多样性 / 结尾账本」，修稿后仍须服从其中的 register/tempo 目标，避免重复同型结尾、同一主角小动作和同一配角画像模板
${lengthGuardrail}
...
${outputFormat}`;
  ...
  private buildRevisionHeatGuard(
    memo: ChapterMemo | undefined,
    intent: ChapterIntent | undefined,
    language: "zh" | "en",
  ): string {
    const heat = resolveChapterHeatTarget(memo, intent);
    if (heat.register === "neutral" && heat.tempo === "medium") return "";
    const craft = renderChapterHeatCraftBlock(heat, language);
    if (language === "en") {
      return `## Chapter Register / Tempo Preservation

Revision must preserve this chapter's register/tempo attempt. Fix review issues, continuity, and AI-tell wording, but do not flatten warm, explosive, bright, or dialogue-driven scenes back into a globally restrained house voice. Book-level style guide and style fingerprint are lower priority than this chapter target.

${craft}`;
    }
    return `## 本章火候保护

修稿必须保留本章 register/tempo 尝试。你要修审稿问题、连续性问题和 AI 味措辞，但不得把温暖、炸裂、明快或对话密的场面按全书克制宪法修回统一腔。全书 style_guide / style fingerprint 的优先级低于本章火候目标。
