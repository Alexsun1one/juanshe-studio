...
export class LengthNormalizerAgent extends BaseAgent {
  ...
  private buildSystemPrompt(mode: LengthNormalizeMode): string {
    ...
    const modeRule = mode === "compress"
      ? `
- 压缩只删冗余：重复解释、同义重述、无功能过渡、可有可无的环境/心理铺陈。**保留所有情节事件、动作、对白、状态变化、钩子和断章。**
- **严禁压过头（铁律）**：成品字数必须落在"允许区间"内，**绝不能低于 softMin/hardMin**；压到 target 附近就停手，不要继续删。宁可略偏上，也绝不砍到区间以下——砍过界会被系统判废、保留原稿，等于白做。`
      : `
- 扩写只补有效内容：真实场景阻力、动作反应、对白承压、必要因果细节。不灌水、不重复、不新增支线或揭示。
- 成品字数必须落在"允许区间"内，尽量贴近 target。`;

    return `你是一位章节长度修正器。你的任务是对章节正文做一次单次修正，只能执行一次，不得递归重写。

修正目标：
- ${action} 章节长度到给定目标区间${modeRule}
...
## Correction Rules
- **落点硬要求**：成品字数必须 ≥ ${input.lengthSpec.softMin} 且 ≤ ${input.lengthSpec.softMax}，尽量贴近目标 ${input.lengthSpec.target}；${mode === "compress" ? `绝不能低于 ${input.lengthSpec.softMin}（压过头会被判废、保留原稿）` : `绝不能高于 ${input.lengthSpec.softMax}`}
- 只修正一次，不要递归
