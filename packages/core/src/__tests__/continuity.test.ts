...
describe("ContinuityAuditor", () => {
  ...
  it("prefers book language override when building audit prompts", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-auditor-lang-test-"));
  ...
  it("localizes English audit prompts instead of mixing Chinese control text", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-auditor-en-prompt-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
  ...
  it("uses selected summary and hook evidence instead of full long-history markdown in governed mode", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-auditor-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
    ...
    try {
      await auditor.auditChapter(
        ...
        "xuanhuan",
        {
          ...
          contextPackage: {
            ...
            selectedContext: [
              {
                source: "story/author_intent.md#core-promise",
                reason: "Preserve the book premise.",
                excerpt: "陆星河核心反差设定，脑内核心能力模块是核心金手指，不能写成普通普通悬疑。",
              },
              {
      ...
      expect(systemPrompt).toContain("主设定保真");
      expect(systemPrompt).toContain("读起来像另一本普通书");
      expect(userPrompt).toContain("story/author_intent.md#core-promise");
      expect(userPrompt).toContain("陆星河核心反差设定");
  ...
  it("injects the chapter memo into the audit prompt for memo-drift checking", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-auditor-memo-drift-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
