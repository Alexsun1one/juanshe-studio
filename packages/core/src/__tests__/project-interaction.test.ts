...
describe("project interaction control", () => {
  beforeAll(async () => {
    projectRoot = await mkdtemp(join(tmpdir(), "autow-project-control-"));
  ...
  it("persists a creation draft across freeform ideation turns", async () => {
    const ideationRoot = await mkdtemp(join(tmpdir(), "autow-project-ideation-"));
    await writeFile(join(ideationRoot, "autow.json"), JSON.stringify({ language: "zh" }), "utf-8");
