...
describe("ComposerAgent", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "autow-composer-test-"));
  ...
  it("keeps meaningful governed excerpts from authoritative files instead of headings or table shells", async () => {
    ...
    await Promise.all([
      ...
      writeFile(
        ...
        [
          ...
          "## 主题与基调",
          "",
          "陆星河是核心反差设定者，脑内残留核心能力模块，优势是模式识别和长期推演。",
        ].join("\n"),
        "utf-8",
      ),
      writeFile(
        ...
        [
          ...
          "| --- | --- |",
          "| 当前章节 | 第44章 |",
          "| 主角状态 | 陆星河赤脚过闸，物证藏在芦苇根部。 |",
        ].join("\n"),
        "utf-8",
      ),
      writeFile(
        join(storyDir, "roles", "主要角色", "陆星河.md"),
        [
          "# 陆星河",
          "",
          "小体量主角，成熟推演心智，核心能力模块不完整、会掉线。",
        ].join("\n"),
        "utf-8",
      ),
    ]);
    ...
    const storyFrame = result.contextPackage.selectedContext.find((entry) =>
      entry.source === "story/outline/story_frame.md",
    );
    const currentState = result.contextPackage.selectedContext.find((entry) =>
      entry.source === "story/current_state.md",
    );
    const roles = result.contextPackage.selectedContext.find((entry) =>
      entry.source === "story/roles",
    );

    expect(authorIntent?.excerpt).toContain("核心反差设定");
    expect(storyFrame?.excerpt).toContain("核心能力模块");
    expect(storyFrame?.excerpt).not.toBe("---");
    expect(currentState?.excerpt).toContain("当前章节");
    expect(currentState?.excerpt).toContain("物证藏在芦苇根部");
    expect(currentState?.excerpt).not.toBe("| 字段 | 值 |");
    expect(roles?.excerpt).toContain("成熟推演心智");
  });
