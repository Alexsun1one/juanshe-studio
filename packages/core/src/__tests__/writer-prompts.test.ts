...
describe("buildWriterSystemPrompt", () => {
  ...
  it("keeps hard guardrails and book/style constraints in governed mode", () => {
    ...
    expect(prompt).toContain("主设定保真约束");
    expect(prompt).toContain("不得只写局部事件而让本书主设定隐身");
  ...
  it("tells governed English prompts to obey variance briefs and include resistance-bearing exchanges", () => {
    ...
    expect(prompt).toContain("Variance Brief");
    expect(prompt).toContain("premise-fidelity constraints");
    expect(prompt).toContain("Do not let local scene mechanics hide the book's defining premise");
    expect(prompt).toContain("resistance-bearing exchange");
  });

  it("tells governed Chinese prompts to avoid repeated chapter templates", () => {
    const prompt = buildWriterSystemPrompt(
      BOOK,
      GENRE,
      null,
    ...
    expect(prompt).toContain("中文变体简报");
