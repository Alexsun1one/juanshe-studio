...
async function createRunnerFixture(
  ...
  const root = await mkdtemp(join(tmpdir(), "autow-runner-test-"));
...
describe("PipelineRunner", () => {
  ...
  it("auto-routes mechanical agents to the model's fastSibling, keeps prose agents on the main model", () => {
    const runner = new PipelineRunner({
      client: {
        provider: "openai",
        apiFormat: "chat",
    ...
    const resolveOverride = (
      runner as unknown as { resolveOverride: (agent: string) => { model: string; client: unknown } }
    ).resolveOverride.bind(runner);

    // 机械型 agent → 自动降配到 fastSibling(更快更省)
    expect(resolveOverride("chapter-analyzer").model).toBe("mimo-v2-flash");
    expect(resolveOverride("length-normalizer").model).toBe("mimo-v2-flash");
    expect(resolveOverride("state-validator").model).toBe("mimo-v2-flash");
    expect(resolveOverride("radar").model).toBe("mimo-v2-flash");

    // 碰 prose / 连贯性裁决的 agent → 保持主模型(质量不降)
    expect(resolveOverride("writer").model).toBe("mimo-v2.5-pro");
    expect(resolveOverride("composer").model).toBe("mimo-v2.5-pro");
    expect(resolveOverride("reviser").model).toBe("mimo-v2.5-pro");
    expect(resolveOverride("polisher").model).toBe("mimo-v2.5-pro");
    expect(resolveOverride("planner").model).toBe("mimo-v2.5-pro");
    expect(resolveOverride("auditor").model).toBe("mimo-v2.5-pro");

    // 复用同一个 client(只换 model 名,不新建连接)
    expect(resolveOverride("chapter-analyzer").client).toBe(resolveOverride("writer").client);
  });

  it("respects explicit overrides over fastSibling auto-routing, and is a no-op when the model has no fastSibling", () => {
    const runner = new PipelineRunner({
      client: {
        provider: "openai",
        apiFormat: "chat",
      ...
      model: "no-such-model-without-card",
      projectRoot: process.cwd(),
      defaultLLMConfig: {
        provider: "custom",
        service: "custom",
      ...
      modelOverrides: {
        // 显式 override 一个机械型 agent → 必须优先于自动 fastSibling 路由
        "chapter-analyzer": "explicitly-chosen-model",
      },
    });
    ...
    // 显式 override 胜出
    expect(resolveOverride("chapter-analyzer").model).toBe("explicitly-chosen-model");
    // 主模型没有 fastSibling 卡 → 机械型 agent 保持主模型(安全降级,不报错)
    expect(resolveOverride("length-normalizer").model).toBe("no-such-model-without-card");
    expect(resolveOverride("writer").model).toBe("no-such-model-without-card");
  });

  it("contentSimilarity scores identical text as 1 and disjoint text near 0", () => {
    const sim = PipelineRunner.contentSimilarity.bind(PipelineRunner);
    expect(sim("完全一样的句子", "完全一样的句子")).toBe(1);
    // 仅润色级改动(增删少量字)→ 高相似(应触发跳过整章重读的阈值 0.9)
    const base = "陆星河盯着监控屏幕，脑海里第二个声音又一次响了起来，冷静而清晰。";
    const polished = "陆星河盯着监控屏幕，脑海里那第二个声音又一次响起，冷静而清晰。";
    expect(sim(base, polished)).toBeGreaterThan(0.5);
    // 完全不同的文本 → 低相似
    expect(sim("abcdefghij", "0123456789")).toBe(0);
    // 空串保护:不崩,不返回 NaN
    expect(sim("", "")).toBe(1);
    expect(sim("", "x")).toBe(0);
  });
  ...
  it("initializes control documents during book creation", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-init-book-test-"));
    ...
    try {
      ...
      expect(styleGuide).toContain("本书文风锚点");
      expect(styleGuide).toContain("Bootstrap Book");
  ...
  it("applies creation-draft overrides while initializing a book", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-init-book-overrides-"));
  ...
  it("honors configured foundation review retry count before blocking a rejected foundation", async () => {
    const { root, runner, bookId } = await createRunnerFixture({
    ...
    try {
      await expect((runner as unknown as {
        ...
        language: "zh",
        stageLanguage: "zh",
      })).rejects.toThrow("基础设定审核 5 轮后仍未通过");
  ...
  it("cleans staged files when initBook fails before foundation is complete", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-init-rollback-"));
    const runner = new PipelineRunner({
      client: {
        provider: "openai",
        apiFormat: "chat",
  ...
  it("replans stale legacy intent artifacts when no freshness snapshot exists in v2 mode", async () => {
    const { root, runner, state, bookId } = await createRunnerFixture({
      inputGovernanceMode: "v2",
    });
    ...
    try {
      ...
      expect(writeInput?.chapterIntent).toContain("test goal");
      expect(writeInput?.chapterIntent).not.toContain("Bring the focus back to the mentor conflict.");
    } finally {
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("normalizes revised output once before re-audit when it leaves the target band", async () => {
    const { root, runner, bookId } = await createRunnerFixture();
    // Over the (widened) hard ceiling so a compress pass triggers; normalized result ≥ hardMin.
    const overlongDraft = "修订后正文。".repeat(100);
    const normalizedDraft = "归一正文。".repeat(45);
  ...
  it("normalizes overlong writer output once before audit", async () => {
    const { root, runner, bookId } = await createRunnerFixture();
    // Over hard-max so a compress pass is triggered; normalized result lands at/above the
    // hard floor so the safety net accepts it (the floor only rejects compresses BELOW hardMin).
    const overlongDraft = "冗余句子。".repeat(800);
    const normalizedDraft = "压缩后的正文。".repeat(330);
  ...
  it("records a length warning when a single normalize pass still misses the hard range", async () => {
    const { root, runner, state, bookId } = await createRunnerFixture();
    // Over the widened ceiling so compress triggers; normalized result still exceeds hardMax (550 @ target 220).
    const overlongDraft = "冗余句子。".repeat(150);
    const stillOverHard = "仍然过长。".repeat(130);
  ...
  it("counts repeated chapter-shape fatigue as a revision blocker", async () => {
    const { root, runner, state, bookId } = await createRunnerFixture();
    const bookDir = state.bookDir(bookId);
    const book = await state.loadBookConfig(bookId);

    await Promise.all([
      writeFile(join(bookDir, "chapters", "0001_早醒.md"), "# 第1章 早醒\n\n天刚蒙蒙亮，陆星河就醒了。他没有惊动沈清禾，摸黑出了院门。到暗渠边时，石灰还沾在鞋底。最后，他把那枚碎片收进袖口。", "utf-8"),
      writeFile(join(bookDir, "chapters", "0002_灰晨.md"), "# 第2章 灰晨\n\n凌晨四点，陆星河睁了眼，先听院里有没有动静。他绕到干渠后面，看见芦苇下压着新脚印。最后，风从渠口吹出来，那点白灰还在发亮。", "utf-8"),
    ]);
    ...
    const result = await (
      ...
      book,
      bookDir,
      chapterContent: "天还没亮透，陆星河在黑暗中睁开眼，没动。他沿着墙根摸到渡口，低头对照泥里的断绳。最后，那块碎木片被水推到脚边。",
      chapterNumber: 3,
      language: "zh",
    });

    expect(result.auditResult.issues.some((issue) => issue.category === "开头场景同构")).toBe(true);
    expect(result.auditResult.issues.some((issue) => issue.category === "章节结构同构")).toBe(true);
    expect(result.revisionBlockingIssues.map((issue) => issue.category)).toEqual(
      expect.arrayContaining(["开头场景同构", "章节结构同构"]),
    );
