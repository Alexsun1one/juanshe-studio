...
describe("buildPlannerUserMessage", () => {
  ...
  it("injects premise fidelity packet from author intent and current focus", () => {
    const out = buildPlannerUserMessage({
      chapterNumber: 45,
      previousChapterEndingExcerpt: "上一章结尾",
      recentSummaries: "",
      currentArcProse: "",
      protagonistMatrixRow: "",
      opponentRows: "",
      ...
      isGoldenOpening: false,
      bookRulesRelevant: "",
      authorIntent: "---\ntitle: x\n---\n陆星河核心反差设定，脑中有核心能力模块和旧世界知识残影。",
      currentFocus: "每章必须显化一个主设定信号，禁止变成普通普通悬疑。",
      storyFrame: "前台是少年主角护家，后台是旧信物、工程系统、远域网络和上界。",
    });

    expect(out).toContain("主设定保真包");
    expect(out).toContain("陆星河核心反差设定");
    expect(out).toContain("核心能力模块");
    expect(out).toContain("每章必须显化一个主设定信号");
