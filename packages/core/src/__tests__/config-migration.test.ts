...
describe("config migration", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "autow-migrate-"));
  });
  ...
  it("migrates old llm.provider+model+apiKey to services[] + secrets", async () => {
    ...
    await writeFile(join(root, "autow.json"), JSON.stringify(oldConfig));

    const result = await migrateConfig(root);

    expect(result.migrated).toBe(true);

    const raw = await readFile(join(root, "autow.json"), "utf-8");
  ...
  it("does nothing if already in new format", async () => {
    ...
    await writeFile(join(root, "autow.json"), JSON.stringify(newConfig));
