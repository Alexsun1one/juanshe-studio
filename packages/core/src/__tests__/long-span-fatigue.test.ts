...
describe("analyzeLongSpanFatigue", () => {
  it("warns when the last three chapter types are identical", async () => {
    const bookDir = await createBookDir("autow-long-span-type-test-");
  ...
  it("warns in English when recent chapter endings are highly similar", async () => {
    const bookDir = await createBookDir("autow-long-span-ending-test-");
  ...
  it("builds an English variance brief with phrase, opening, ending, and scene guidance", async () => {
    const bookDir = await createBookDir("autow-variance-brief-test-");
  ...
  it("warns when Chinese openings repeat the same wake-up scene shape", async () => {
    const bookDir = await createBookDir("autow-long-span-zh-opening-scene-test-");

    await Promise.all([
      writeChapter(bookDir, 1, "早醒", "天刚蒙蒙亮，陆星河就醒了。他没有惊动沈清禾，摸黑出了院门。到暗渠边时，石灰还沾在鞋底。最后，他把那枚碎片收进袖口。"),
      writeChapter(bookDir, 2, "灰晨", "凌晨四点，陆星河睁了眼，先听院里有没有动静。他绕到干渠后面，看见芦苇下压着新脚印。最后，风从渠口吹出来，那点白灰还在发亮。"),
    ]);
    ...
    try {
      ...
      expect(result.issues.some((issue) => issue.category === "开头场景同构")).toBe(true);
      expect(result.issues.some((issue) => issue.category === "章节结构同构")).toBe(true);
      expect(result.issues.find((issue) => issue.category === "开头场景同构")?.suggestion).toContain("禁止再用凌晨/醒来");
    } finally {
      await rm(join(bookDir, ".."), { recursive: true, force: true });
    }
  });

  it("builds a Chinese variance brief that forbids the repeated wake-and-scout template", async () => {
    const bookDir = await createBookDir("autow-zh-variance-brief-test-");

    await Promise.all([
      writeChapter(bookDir, 1, "早醒", "天刚蒙蒙亮，陆星河就醒了。他没动，听了一阵，随后出了院门去暗渠边看脚印。最后，他把碎片收进袖口。"),
      writeChapter(bookDir, 2, "灰晨", "凌晨四点，陆星河睁开眼。他听了一阵院里的动静，绕到芦苇边捡起沾着石灰的碎片。最后，夜色压在渠口。"),
    ]);

    try {
      const brief = await buildVarianceBrief({
        bookDir,
        chapterNumber: 3,
        language: "zh",
      });

      expect(brief?.text).toContain("## 中文变体简报");
      expect(brief?.repeatedOpeningPatterns).toContain("凌晨/天亮醒来开场");
      expect(brief?.sceneObligation).toContain("必须换开篇入口");
      expect(brief?.text).toContain("醒来 → 出门探查 → 发现物件");
    } finally {
      await rm(join(bookDir, ".."), { recursive: true, force: true });
    }
  });
  ...
  it("warns when title focus collapses and high-tension mood never releases", async () => {
    const bookDir = await createBookDir("autow-long-span-cadence-test-");
