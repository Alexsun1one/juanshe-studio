...
describe("analyzeAITells", () => {
  ...
  it("detects telling-not-showing emotion naming (dim 24)", () => {
    const content = "他感到恐惧。过了一会儿，他又感到愤怒。然后他感到紧张，最后感到绝望。";
    const result = analyzeAITells(content);
    const tellIssues = result.issues.filter((i) => i.category === "直白命名情绪");
    expect(tellIssues.length).toBeGreaterThan(0);
    expect(tellIssues[0]!.severity).toBe("warning");
  });

  it("detects cliché imagery / purple-prose phrases (dim 25)", () => {
    const content = "空气仿佛凝固了。四周鸦雀无声。他嘴角勾起一抹弧度，眼底闪过一丝寒意。";
    const result = analyzeAITells(content);
    const clicheIssues = result.issues.filter((i) => i.category === "套话意象");
    expect(clicheIssues.length).toBeGreaterThan(0);
    expect(clicheIssues[0]!.severity).toBe("warning");
  });

  it("does not flag a single incidental cliché as an issue", () => {
    const content = "他盯着对方，眼神里有一丝不易察觉的警惕，然后转身走开了。";
    const result = analyzeAITells(content);
    const clicheIssues = result.issues.filter((i) => i.category === "套话意象");
    expect(clicheIssues).toHaveLength(0);
  });
