...
describe("resolveEffectiveLLMConfig", () => {
  ...
  async function writeProject(llm: Record<string, unknown>) {
    root = await mkdtemp(join(tmpdir(), "autow-effective-llm-"));
  ...
  async function writeSecrets(services: Record<string, { apiKey: string }>) {
    ...
    await writeFile(join(root, ".autow", "secrets.json"), JSON.stringify({ services }, null, 2), "utf-8");
  }
  ...
  it("CLI consumer 兼容旧 env：没有 AUTOW_LLM_SERVICE 时从 baseUrl 反推 service", async () => {
    await writeProject({
      configSource: "studio",
      service: "google",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          AUTOW_LLM_SERVICE: "moonshot",
          AUTOW_LLM_MODEL: "kimi-k2.5",
          AUTOW_LLM_API_KEY: "sk-global-moon",
        },
        project: {
          AUTOW_LLM_SERVICE: "deepseek",
          AUTOW_LLM_MODEL: "deepseek-chat",
          AUTOW_LLM_API_KEY: "sk-project-deepseek",
        },
        process: {
          AUTOW_LLM_SERVICE: "zhipu",
          AUTOW_LLM_MODEL: "glm-4-flash",
          AUTOW_LLM_API_KEY: "sk-process-zhipu",
        },
      },
      requireApiKey: true,
    });
  ...
  it("旧 configSource=env 保持 legacy-env 行为", async () => {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          AUTOW_LLM_PROVIDER: "custom",
          AUTOW_LLM_BASE_URL: "https://api.example.com/v1",
          AUTOW_LLM_MODEL: "legacy-model",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          ...
          AUTOW_LLM_MODEL: "legacy-model",
          AUTOW_LLM_API_KEY: "sk-env",
          AUTOW_LLM_API_FORMAT: "chat",
          AUTOW_LLM_STREAM: "true",
        },
        project: {},
        process: {},
      },
  ...
  it("保留旧 AUTOW_LLM_EXTRA_* 和 AUTOW_DEFAULT_LANGUAGE 行为", async () => {
    await writeProject({
      configSource: "env",
      provider: "openai",
      baseUrl: "https://api.example.com/v1",
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        ...
        project: {
          AUTOW_LLM_API_KEY: "sk-env",
          AUTOW_LLM_EXTRA_top_p: "0.9",
          AUTOW_DEFAULT_LANGUAGE: "en",
        },
        process: {},
      },
    });
  ...
  it("CLI override 优先级高于 env", async () => {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {},
        project: { AUTOW_LLM_SERVICE: "google", AUTOW_LLM_MODEL: "gemini-2.5-pro" },
        process: {},
      },
  ...
  it("拒绝不属于最终 service 的模型", async () => {
    ...
    await expect(resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {},
        project: { AUTOW_LLM_MODEL: "kimi-k2.5" },
        process: {},
      },
  ...
  it("CLI env 指向 Ollama 时允许用户本地安装的动态模型", async () => {
    ...
    const result = await resolveEffectiveLLMConfig({
      ...
      envLayers: {
        global: {
          AUTOW_LLM_SERVICE: "ollama",
          AUTOW_LLM_PROVIDER: "openai",
          AUTOW_LLM_BASE_URL: "http://127.0.0.1:11434/v1",
          AUTOW_LLM_MODEL: "qwen3.6:35b-a3b",
        },
        project: {},
        process: {},
      },
