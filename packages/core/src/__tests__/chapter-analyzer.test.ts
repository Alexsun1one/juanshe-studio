...
describe("ChapterAnalyzerAgent", () => {
  ...
  it("counts English chapter content using words instead of characters", async () => {
    const bookDir = await mkdtemp(join(tmpdir(), "autow-chapter-analyzer-"));
  ...
  it("uses English prompts when analyzing imported English chapters", async () => {
    const bookDir = await mkdtemp(join(tmpdir(), "autow-chapter-analyzer-en-"));
    const englishContent = "He looked at the sky and waited.";
    const agent = new ChapterAnalyzerAgent({
      client: {
        provider: "openai",
    ...
    try {
      ...
      expect(flat(messages[0])).toContain("ALL output MUST be in English");
      expect(flat(messages[1])).toContain("Analyze chapter 1");
      expect(flat(messages[1])).toContain("## Chapter Content");
      expect(flat(messages[1])).toContain("## Current State");
      expect(flat(messages[1])).not.toContain("请分析第1章正文");
    } finally {
      await rm(bookDir, { recursive: true, force: true });
    }
  });
  ...
  it("uses a retrieved summary snapshot instead of full long-history chapter summaries", async () => {
    const bookDir = await mkdtemp(join(tmpdir(), "autow-chapter-analyzer-memory-"));
  ...
  it("preserves the supplied chapter content when the model omits CHAPTER_CONTENT", async () => {
    const bookDir = await mkdtemp(join(tmpdir(), "autow-chapter-analyzer-fallback-"));
  ...
  it("uses governed control inputs instead of old broad truth-file blocks when provided", async () => {
    const bookDir = await mkdtemp(join(tmpdir(), "autow-chapter-analyzer-governed-"));
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
