...
describe("persisted-governed-plan round trip", () => {
  it("savePersistedPlan + loadPersistedPlan returns equal memo", async () => {
    const dir = await mkdtemp(join(tmpdir(), "autow-plan-"));
    await mkdir(join(dir, "story", "runtime"), { recursive: true });
    await writePlannerInputs(dir, "volume v1", "state v1");
...
`;
  ...
  it("returns null when any planner input changed after memo persistence", async () => {
    const dir = await mkdtemp(join(tmpdir(), "autow-plan-"));
    await mkdir(join(dir, "story", "runtime"), { recursive: true });
    await writePlannerInputs(dir, "volume v1", "state v1");
    await writeFile(
    ...
    await writePlannerInputs(dir, "volume v2 after current_focus repair", "state v1");
...
async function writePlannerInputs(dir: string, volume: string, state: string) {
  await mkdir(join(dir, "story"), { recursive: true });
  await writeFile(join(dir, "story", "volume_outline.md"), volume, "utf-8");
  await writeFile(join(dir, "story", "current_state.md"), state, "utf-8");
}
