...
describe("retrieveMemorySelection", () => {
  ...
  it("indexes current state facts into sqlite-backed memory selection", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-test-"));
  ...
  it("prefers the mentor-debt recap chapter over nearby guild-noise chapters in English retrieval", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-en-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
  ...
  it("prefers the explicit 师债回响 chapter over nearby 商会噪音 chapters in Chinese retrieval", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-zh-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
  ...
  it("keeps the mentor-debt recap chapter in markdown fallback mode for English books", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-en-fallback-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
  ...
  it("keeps the 师债回响 chapter in markdown fallback mode for Chinese books", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-zh-fallback-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
  ...
  sqliteIt("uses existing sqlite summaries and hooks without requiring markdown truth files", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-db-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
  ...
  sqliteIt("backfills sqlite memory from structured state instead of stale markdown truth files", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-db-structured-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
  ...
  it("bootstraps structured runtime state from legacy markdown truth files during retrieval", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-bootstrap-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const stateDir = join(storyDir, "state");
    await mkdir(storyDir, { recursive: true });
  ...
  it("prefers structured state files over legacy markdown truth files when both exist", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-structured-preferred-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const stateDir = join(storyDir, "state");
    await mkdir(stateDir, { recursive: true });
  ...
  it("recalls stale open hooks alongside recent governed memory selections", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-stale-hook-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const stateDir = join(storyDir, "state");
    await mkdir(stateDir, { recursive: true });
  ...
  it("surfaces one stale unresolved hook beyond the primary quota while excluding stale resolved hooks", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-stale-quota-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const stateDir = join(storyDir, "state");
    await mkdir(stateDir, { recursive: true });
  ...
  it("surfaces multiple stale hook families when debt pressure clusters instead of only one stale extra", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-stale-cluster-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const stateDir = join(storyDir, "state");
    await mkdir(stateDir, { recursive: true });
  ...
  it("does not surface far-future unstarted hooks in early chapter retrieval", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-future-hook-gate-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const stateDir = join(storyDir, "state");
    await mkdir(stateDir, { recursive: true });
  ...
  it("does not resurface a resolved hook just because mustKeep shares an artifact term", async () => {
    root = await mkdtemp(join(tmpdir(), "autow-memory-retrieval-resolved-artifact-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const stateDir = join(storyDir, "state");
    await mkdir(stateDir, { recursive: true });
...
describe("retrieveMemorySelection · 长篇摘要视野", () => {
  ...
  it("uses a 30-chapter semantic candidate pool but still injects only 8 compressed summaries", async () => {
    rootDir = await mkdtemp(join(tmpdir(), "autow-long-summary-window-"));
    const bookDir = join(rootDir, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });

    const rows = Array.from({ length: 45 }, (_value, index) => {
      const chapter = index + 1;
      const event = chapter === 20
        ? "River scar clue returns without lexical overlap"
        : `Routine pressure beat ${chapter}`;
      return `| ${chapter} | Title ${chapter} | Lin Yue | ${event} | State ${chapter} | Hook ${chapter} | tense | mainline |`;
    });
    ...
    const embed = async (texts: ReadonlyArray<string>): Promise<number[][]> =>
      texts.map((text, index) => {
        if (index === 0) return [1, 0];
        return text.includes("River scar clue") ? [1, 0] : [0, 1];
      });

    const result = await retrieveMemorySelection({
      bookDir,
      chapterNumber: 46,
      goal: "Settle the hidden debt without naming any river clue.",
      embed,
    });

    expect(result.summaries.length).toBeLessThanOrEqual(8);
    expect(result.summaries.map((summary) => summary.chapter)).toContain(20);
  });
});
