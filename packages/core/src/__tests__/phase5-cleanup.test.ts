...
describe("Phase 5 cleanup (1) — volume_outline.md mirror removed", () => {
  ...
  beforeEach(async () => {
    bookDir = await mkdtemp(join(tmpdir(), "autow-cleanup-1-"));
  });
  ...
  it("isCompleteBookDirectory accepts the new outline/ layout (no legacy mirror needed)", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "autow-cleanup-1-proj-"));
    try {
      ...
      await Promise.all([
        ...
        writeFile(join(storyDir, "roles/主要角色/主角.md"), "# 主角", "utf-8"),
        writeFile(join(storyDir, "book_rules.md"), "# rules", "utf-8"),
        writeFile(join(storyDir, "style_guide.md"), "# style", "utf-8"),
  ...
  it("isCompleteBookDirectory rejects new outline/ books without style guide and role cards", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "autow-cleanup-1-incomplete-"));
    try {
      const targetBookDir = join(projectRoot, "books", "incomplete-book");
      const storyDir = join(targetBookDir, "story");
      await mkdir(join(storyDir, "outline"), { recursive: true });
      await mkdir(join(targetBookDir, "chapters"), { recursive: true });
  ...
  it("isCompleteBookDirectory still accepts the pre-cleanup layout (legacy flat files)", async () => {
    const projectRoot = await mkdtemp(join(tmpdir(), "autow-cleanup-1-legacy-"));
    try {
...
describe("Phase 5 cleanup (2) — architect no longer seeds runtime log files", () => {
  ...
  beforeEach(async () => {
    bookDir = await mkdtemp(join(tmpdir(), "autow-cleanup-2-"));
  });
...
describe("Phase 5 cleanup (3) — book_rules YAML moved to story_frame.md frontmatter", () => {
  ...
  beforeEach(async () => {
    bookDir = await mkdtemp(join(tmpdir(), "autow-cleanup-3-"));
  });
