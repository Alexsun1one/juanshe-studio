...
describe("WriterAgent", () => {
  ...
  it("renders per-chapter user context in governed creative prompts", () => {
    const agent = new WriterAgent({
      ...
      model: "test-model",
      projectRoot: "/tmp/autow-writer-context-test",
    });
  ...
  it("uses compact summary context plus selected long-range evidence during governed settlement", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-writer-test-"));
  ...
  it("builds structured runtime-state artifacts when settler returns a delta", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-writer-runtime-state-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
  ...
  it("overrides hallucinated chapter numbers across both delta and summary row", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-writer-runtime-state-hallucinated-chapter-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const chaptersDir = join(bookDir, "chapters");
    await mkdir(storyDir, { recursive: true });
  ...
  it("returns the arbiter-resolved delta instead of raw new-hook candidates", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-writer-arbiter-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const chaptersDir = join(bookDir, "chapters");
    await mkdir(storyDir, { recursive: true });
  ...
  it("degrades settlement instead of hanging when truth merge stalls", async () => {
    const previousTimeout = process.env.HARDWRITE_SETTLEMENT_CHAT_TIMEOUT_MS;
    process.env.HARDWRITE_SETTLEMENT_CHAT_TIMEOUT_MS = "5";

    const root = await mkdtemp(join(tmpdir(), "autow-writer-settlement-timeout-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    ...
    try {
      ...
      expect(output.postSettlement).toContain("状态结算降级");
      expect(output.updatedState).toContain("林越还在破庙外");
      expect(output.updatedHooks).toContain("mentor-oath");
      expect(output.chapterSummary).toContain("| 2 | 迟到的火光 |");
      expect(warnings.some((warning) => warning.includes("真相回写超时/失败"))).toBe(true);
    } finally {
      if (previousTimeout === undefined) {
        delete process.env.HARDWRITE_SETTLEMENT_CHAT_TIMEOUT_MS;
      } else {
        process.env.HARDWRITE_SETTLEMENT_CHAT_TIMEOUT_MS = previousTimeout;
      }
      await rm(root, { recursive: true, force: true });
    }
  });
  ...
  it("injects an English variance brief into governed creative prompts", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-writer-variance-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    const chaptersDir = join(bookDir, "chapters");
    await mkdir(storyDir, { recursive: true });
  ...
  it("renders explicit title history, mood trail, and canon blocks in governed creative prompts", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-writer-governed-evidence-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
  ...
  it("sanitizes governed control inputs so raw hook ids and control headings do not enter the creative prompt", async () => {
    const root = await mkdtemp(join(tmpdir(), "autow-writer-hook-agenda-test-"));
    const bookDir = join(root, "book");
    const storyDir = join(bookDir, "story");
    await mkdir(storyDir, { recursive: true });
