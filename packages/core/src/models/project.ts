...
// 每个模型的真实 maxOutput 来自 providers/<name>.ts 的 HardWriteModel.maxOutput；
...
/** 正文写作阶段配置。reviewRetries：章节评分/修复循环的最大轮次（可调，默认保持 3 不降质）。 */
export const WritingConfigSchema = z.object({
  reviewRetries: z.number().int().min(0).max(10).default(3),
});
