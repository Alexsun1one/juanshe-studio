...
async function readDir(projectRoot: string, dir: string): Promise<SecretsFile | null> {
  try {
    const raw = await readFile(join(projectRoot, dir, SECRETS_FILE), "utf-8");
    const parsed = JSON.parse(raw) as SecretsFile;
    if (!parsed || typeof parsed !== "object" || !parsed.services) return null;
    return parsed;
  } catch {
    return null;
  }
...
async function readSecretsRaw(projectRoot: string): Promise<SecretsFile> {
  const primary = await readDir(projectRoot, SECRETS_DIR);
