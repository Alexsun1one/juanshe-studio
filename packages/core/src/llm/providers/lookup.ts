import type { HardWriteModel } from "./types.js";
...
export function lookupModel(
  ...
  const matches: Array<{ model: HardWriteModel; providerId: string }> = [];
...
export function listEnabledModels(serviceId: string): HardWriteModel[] {
  const provider = getEndpoint(serviceId);
  if (!provider) return [];
  return provider.models.filter((m) => m.enabled !== false);
}

export function isActiveTextModel(model: HardWriteModel): boolean {
...
export function listActiveTextModels(serviceId: string): HardWriteModel[] {
  const provider = getEndpoint(serviceId);
  if (!provider) return [];
