/**
 * HardWrite 自维护的 provider 定义。每个 provider 一个 .ts 文件，
 * 里面一个 HardWriteEndpoint 对象（provider 元数据 + models 数组）。
 *
 * 数据冷启动自 lobe-chat/packages/model-bank，之后由 HardWrite 自管。
...
export interface HardWriteModel {
  ...
  /**
   * 同源「快/省」兄弟模型 id。声明后，pipeline 里只做结构化抽取 / 字数归一 / 状态校验 /
   * 趋势扫描的「快轨」agent 会自动降配到它（不碰 prose 质量与连贯性裁决），更快更省。
   * 只在「质量/旗舰」模型卡上声明，指向它的 flash 版；flash 模型自身不要设。
   * 这样「一处声明、惠及所有书」，且不把具体 flash 名字硬编码进 pipeline。
   */
  readonly fastSibling?: string;
  /**
...
export interface HardWriteEndpoint {
  readonly id: string;
  ...
  readonly models: readonly HardWriteModel[];
}
