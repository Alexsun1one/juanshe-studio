...
export type { HardWriteEndpoint, HardWriteModel, ApiProtocol, EndpointGroup } from "./types.js";

/**
 * 所有已注册 provider 的扁平列表。顺序定义了 lookup Layer 2 的遍历顺序，
 * 但 Layer 2 还会按 PROVIDER_PRIORITY 显式排序，所以此处顺序不影响结果。
 */
const ALL_PROVIDERS: readonly HardWriteEndpoint[] = [
...
const PROVIDERS_BY_ID: Map<string, HardWriteEndpoint> = new Map(
  ALL_PROVIDERS.map((p) => [p.id, p]),
);

export function getAllEndpoints(): readonly HardWriteEndpoint[] {
  return ALL_PROVIDERS;
}

export function getEndpoint(id: string): HardWriteEndpoint | undefined {
