...
export function resolveServicePreset(service: string): ServicePreset | undefined {
  ...
  return {
    ...
    // piProvider 字段已从 HardWriteEndpoint 移除（走 provider-to-pi-ai adapter），这里只保留 legacy fallback
...
function toModelInfo(hardWriteModel: { id: string; maxOutput: number; contextWindowTokens: number }): ModelInfo {
  return {
    id: hardWriteModel.id,
    name: hardWriteModel.id,
    contextWindow: hardWriteModel.contextWindowTokens,
    maxOutput: hardWriteModel.maxOutput,
  };
}
 ...
 * - probe 失败或无 apiKey：fallback 到 provider.models（HardWrite bank）
 * - 不再做 AUTOW_LLM_MODEL env 补丁（会污染跨 service 菜单；bank 已足够全）
 *
