...
vi.mock("@juanshe/core", async (importOriginal) => {
  ...
  class MockStateManager {
    ...
    async saveBookConfig(_bookId: string, _config: unknown): Promise<void> {
      // no-op in tests
    }
  ...
  return {
    ...
    GLOBAL_ENV_PATH: join(tmpdir(), "hardwrite-global.env"),
  };
});
...
async function waitForCreateStatus(
  ...
  for (let attempt = 0; attempt < 50; attempt += 1) {
    const response = await app.request(`http://localhost/api/v1/books/${bookId}/create-status`);
  ...
  throw new Error(`Timed out waiting for ${bookId} create status: ${JSON.stringify(lastBody)}`);
}
...
describe("createStudioServer daemon lifecycle", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "hardwrite-studio-server-"));
    ...
    loadProjectConfigMock.mockImplementation(async () => {
      const raw = JSON.parse(await readFile(join(root, "hardwrite.json"), "utf-8")) as Record<string, unknown>;
      return {
        ...cloneProjectConfig(),
        ...raw,
        llm: {
  ...
  it("uses the real core bookId validator in the Studio safety mock", async () => {
    const { isSafeBookId } = await import("@juanshe/core");
  ...
  it("returns from /api/daemon/start before the first write cycle finishes", async () => {
    ...
    await expect(status.json()).resolves.toMatchObject({ running: true });
  ...
  it("reports config source and detected env overrides for Studio switching", async () => {
    await writeFile(join(root, ".env"), [
      "HARDWRITE_LLM_PROVIDER=openai",
      "HARDWRITE_LLM_BASE_URL=https://project.example.com/v1",
      "HARDWRITE_LLM_MODEL=gpt-5.4",
      "HARDWRITE_LLM_API_KEY=sk-project",
    ].join("\n"), "utf-8");
    await writeFile(join(tmpdir(), "hardwrite-global.env"), [
      "HARDWRITE_LLM_PROVIDER=openai",
      "HARDWRITE_LLM_BASE_URL=https://global.example.com/v1",
      "HARDWRITE_LLM_MODEL=gpt-4o",
      "HARDWRITE_LLM_API_KEY=sk-global",
    ].join("\n"), "utf-8");
    await writeFile(join(root, "hardwrite.json"), JSON.stringify({
      ...projectConfig,
      llm: {
  ...
  it("auto-detects a working custom combination when /models is unavailable", async () => {
    ...
    await writeFile(join(root, ".env"), [
      "HARDWRITE_LLM_MODEL=MiniMax-M2.7",
      "HARDWRITE_LLM_BASE_URL=https://api.minimax.com/v1",
      "HARDWRITE_LLM_API_KEY=sk-minimax",
    ].join("\n"), "utf-8");
  ...
  it("never returns the plaintext service secret (write-only; only hasKey + masked tail)", async () => {
    loadSecretsMock.mockResolvedValue({
      services: {
    ...
    expect(response.status).toBe(200);
    // 安全契约:GET 绝不回明文 key,只回"已配置 + 末4位掩码"(防跨域窃取)
    await expect(response.json()).resolves.toEqual({ hasKey: true, masked: "••••moon" });
  });
  ...
  it("returns fallback book description when the selected book config is missing", async () => {
    loadBookConfigMock.mockRejectedValue(new Error("ENOENT: no such file or directory, open '/tmp/books/stale-book/book.json'"));

    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);

    const response = await app.request("http://localhost/api/v1/books/stale-book/description");
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      missingBook: true,
      description: {
        oneLine: expect.stringContaining("stale-book"),
        tags: ["长篇小说"],
      },
    });
  });
  ...
  it("reports async create failures through the create-status endpoint", async () => {
    processProjectInteractionRequestMock.mockRejectedValueOnce(new Error("AUTOW_LLM_API_KEY not set"));
    ...
    expect(response.status).toBe(200);
    const status = await waitForCreateStatus(app, "broken-book", (body) => body.status !== "creating");
    expect(status).toMatchObject({
      status: "needs-foundation",
      error: "AUTOW_LLM_API_KEY not set",
      failureReason: expect.stringContaining("模型凭证或服务鉴权失败"),
    });
  });

  it("recovers create-status from a persisted outlining book after backend restart", async () => {
    const restoredBook = {
      id: "restored-book",
      title: "Restored Book",
      platform: "qidian",
    ...
    loadBookConfigMock.mockImplementation(async (bookId?: string) => {
      if (bookId === "restored-book") return restoredBook;
      return {
        id: "demo-book",
        title: "Demo Book",
        platform: "qidian",
    ...
    const response = await app.request("http://localhost/api/v1/books/restored-book/create-status");
    ...
    await expect(response.json()).resolves.toMatchObject({
      ...
      recovered: true,
      failureReason: expect.stringContaining("持久化书籍文件恢复状态"),
  ...
  it("does not mark recovered outlining create runs as done", async () => {
    const restoredBook = {
      id: "restored-book",
      title: "Restored Book",
      platform: "qidian",
    ...
    await mkdir(join(root, "books", "restored-book"), { recursive: true });
    await writeFile(join(root, "books", "restored-book", "book.json"), JSON.stringify(restoredBook), "utf-8");
    await writeFile(join(root, ".hardwrite", "task_runs.json"), JSON.stringify({
      version: 1,
      updatedAt: "2026-04-12T00:00:00.000Z",
      runs: [{
        id: "run-restored",
    ...
    const response = await app.request("http://localhost/api/v1/runs?bookId=restored-book");
    ...
    expect(payload.runs).toEqual([
      expect.objectContaining({
        ...
        currentStage: "建书未完成，已保存可恢复草稿",
        error: "Book foundation is incomplete.",
        failureReason: expect.stringContaining("Book foundation is incomplete"),
        events: expect.arrayContaining([
          expect.objectContaining({
            kind: "book:needs-foundation",
            stage: "建书未完成，已保存可恢复草稿",
  ...
  it("routes create requests through the shared structured interaction runtime", async () => {
    ...
    expect(processProjectInteractionRequestMock).toHaveBeenCalledWith(expect.objectContaining({
      ...
      request: expect.objectContaining({
        ...
        authorIntent: expect.stringContaining("主角在旧城查账洗白"),
        currentFocus: expect.stringContaining("主角在旧城查账洗白"),
        worldPremise: expect.stringContaining("主角在旧城查账洗白"),
        platformGuidance: expect.any(String),
        platformProfile: expect.any(Object),
      }),
    }));
    await waitForCreateStatus(app, "new-book", (body) => body.status === "created");
  });
  ...
  it("creates books with Studio Ollama config without requiring an API key", async () => {
    processProjectInteractionRequestMock.mockResolvedValueOnce({
      ...
      details: {
        bookId: "local-book",
        outputPath: join(root, "books", "local-book", "local-book.txt"),
        chaptersExported: 0,
      },
    });
    ...
    await waitForCreateStatus(app, "local-book", (body) => body.status === "created");
  });
  ...
  it("serves export downloads with RFC 5987 filenames for non-ASCII book ids", async () => {
    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);
    const bookId = "星尘邮局";

    await mkdir(join(root, "books", bookId, "chapters"), { recursive: true });
    await writeFile(join(root, "books", bookId, "chapters", "0001_第一封迟到的信.md"), "# 第一封迟到的信\n\n正文第一段", "utf-8");
    ...
    loadChapterIndexMock.mockResolvedValueOnce([
      { number: 1, status: "ready-for-review", wordCount: 4 },
    ]);

    const response = await app.request(`http://localhost/api/v1/books/${encodeURIComponent(bookId)}/export?format=txt`);
  ...
  it("blocks write-next chat instructions from running the writer pipeline", async () => {
    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);
    ...
    expect(response.status).toBe(400);
    await expect(response.json()).resolves.toMatchObject({
      response: expect.stringContaining("写作类指令请使用工作台"),
      session: {
        sessionId: "agent-session-1",
        activeBookId: "demo-book",
      },
      error: {
        code: "WRITE_FROM_CHAT_DISABLED",
        message: expect.stringContaining("写作类指令请使用工作台"),
      },
    });
  ...
  it("allows /api/agent to use explicit service+model when Studio config has no defaultModel", async () => {
    await writeFile(join(root, "hardwrite.json"), JSON.stringify({
      ...
      llm: {
        configSource: "studio",
        services: [
          { service: "custom", name: "ExampleGateway", baseUrl: "https://llm.example.com/v1", apiFormat: "responses", stream: false },
        ],
      },
    }, null, 2), "utf-8");
    loadProjectConfigMock.mockImplementation(async () => {
  ...
  it("creates a Studio agent session when sessionId is omitted", async () => {
    const { createStudioServer } = await import("./server.js");
    const app = createStudioServer(cloneProjectConfig() as never, root);
    ...
    const response = await app.request("http://localhost/api/v1/agent", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ instruction: "检查当前状态", activeBookId: "demo-book" }),
    });

    expect(response.status).toBe(200);
    expect(createAndPersistBookSessionMock).toHaveBeenCalledWith(root, "demo-book");
