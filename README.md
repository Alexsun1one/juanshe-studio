...
- **去 AI 味质量门禁**：零成本 L0 机检直接拦「不是A而是B」等高频 AI 句式，词表区分硬伤与正常叙事词；判官按「开篇有没有钩、章末断没断在动作上、配角台词遮名能不能分清」打分；全书级复读账本记下用滥的表达，下一章自动禁用。落盘前还有最后一道硬禁令校验，章末预言句这类批量货进不了库。
- **沉浸阅读**：宋体两端对齐的出版级排版、人物 / 地点 / 对话语义分色、场景分隔符、上一章 / 下一章、字号行宽行高三档偏好；AI 正在写这本书时，正文会以流式逐段长出来。
- **挂机连写的安心感**：工作台与剧场直接显示批次进度（第 X/N 章 · 还剩 · 预计时间），流式画布支持上翻回读不被拽走、断线刷新后半章正文自动恢复；切到后台时标签页标题和系统通知会替你盯着「章节完成 / 运行失败 / 低于门槛」。
...
- [x] 写作质量管线 v2：规划契约对齐（蓝图不再静默降级）、润色走定点补丁（PATCH）不再整章重写、连写时上一章结尾原文注入下一章写手。
- [x] 去 AI 味检测 v2：禁用句式正则机检、词表硬软分层与密度口径修正、judge 追读 / 钩子 / 人物声音判据、全书复读账本、落盘硬禁令终检。
- [x] 沉浸阅读升级：语义分色、出版级排版、流式活水、章间导航与阅读位置记忆、阅读偏好三档。
- [x] 全站暖纸像素皮收口：冷蓝 / 深棕 token 清洗、⌘K 命令面板 / toast / 骨架屏 / 按钮三态统一皮肤、迷你空态、主题预览样片画真。
- [x] 长任务体验：批次进度微组件、失败原因与恢复入口、流式礼貌跟随与断线恢复、数字缓动、批准 / 完本庆祝、标签页标题与系统通知。
- [x] 全本导出 Markdown / TXT 双格式与排版清洗（章题统一、剥 markdown 残留、场景分隔符）。
...
- [ ] 质量分新口径的真书观察：检测阈值已集中到一处可调，需要跑一两本书看返修率和误杀率，再决定是否联调 85 分门禁。
- [ ] audit-failed 章（带病落盘）的前端醒目标识与一键复修引导。
- [ ] 像素 UI 字体 unicode-range 切片（602KB 单体 → 常用字首片），治首访字形跳变。
- [ ] EPUB 导出在 UI 暴露（后端清洗链路已就绪）。

### 下一步

- [ ] 语义向量检索：在现有词面检索 + 图谱遍历之外，接入 embedding 模型。
- [ ] 局部干预：重写半章、级联更新记忆、保留改动证据。
- [ ] 风格指纹接线：StyleProfile 子系统已有实现但未进写手提示词与签发评分，接通后可做「越写越像这本书自己」的风格收敛。
- [ ] 开篇 / 章末钩子的确定性机检：目前靠判官提示词约束，补一层零成本启发式检测兜底。
- [ ] 互动小说 / 分支叙事。
