...
把卷舍从"本地单机"跑成"托管多租户网页版":用户开浏览器 → 邮箱密码注册 → 免码即写(轻档)→ 输 Pro/Ultra 升级码解锁中/重档。复用 `your-vps`(与发卡服务 `/opt/your-activation-service` 共机)。
...
2. **`HARDWRITE_ACTIVATION_SECRET` 与发卡服务逐字一致**(`/opt/your-activation-service/.env` 里那把),否则用户 Pro/Ultra 码在 SaaS 这边验不过。
3. **`HARDWRITE_COOKIE_SECURE=1` + 全程 HTTPS**(compose 已设)。HTTP 下 Secure cookie 不回传 = 登录形同虚设。
4. **端口/域名不撞发卡服务**:`your-domain.example` 已占宿主 80/443。见第 3 节——**共机别用 compose 自带 nginx,挂到宿主 nginx**。
...
`your-vps` 已有宿主 nginx 在跑 `your-domain.example`。**注释掉 compose 里的 `nginx` 服务**,让 web 只在内网暴露,由宿主 nginx 加一个 server block 反代:

```nginx
# 宿主 /etc/nginx/sites-available/juanshe-saas.conf — 新子域,如 your-domain.example
server {
    listen 443 ssl;
    server_name your-domain.example;             # ← 你的 SaaS 子域
...
公众号「领码」自动回复改成:发 **SaaS 访问链接(your-domain.example)+ 一个限时 Pro 体验码**。用户开链接免码注册即写,Pro 码升级 —— 门槛归零。发卡服务铸限时 Pro 码的逻辑在 `/opt/your-activation-service`(VPS,独立改)。
